<%@ Page Language="C#" AutoEventWireup="true" CodeFile="run-stand-4landuse.aspx.cs"
    Inherits="run_stand_4landuse" %>
    ...
    <div id="mainPageDiv">
        ...
        <div id="mainPageTitle" style="margin-left: 20px">
            <span><a href="javascript:insertContent('stepNotes', 'notesView', 'notesHide')" id="notesView">
                ...
                Hide</a> notes for this step </span>Future Management and Disturbance Regime - Stand
        </div>
        <div id="main">
            <div id="stepNotes" style="display: none">
                This page describes how you plan to manage the stand in the future. For each management step select a treatment interval, treatment regime type, percent disturbed 
                and utilization / severity before clicking the 'Add ' button. Individual events will then appear in the events text box below. Treatment types include harvests of 
                various types (stem only, aboveground, whole tree, and salvage), prescribed fires, and wildfires. Percent disturbed and utilization / severity are used exactly the 
                same as for Disturbance History. The 'Primary Regime' is the main reoccurring management action and this will occur starting from the Reference Calendar Year selected 
                in simulation characteristics. One or more 'Secondary Regimes', may be defined by entering the number of years from the primary treatment (offset years). For example, 
                if a prescribed fire follows a harvest, indicating a 1 year treatment offset will make fires occur one year after each harvest. You can confirm the treatment sequence 
                in the text box below. You can remove certain events by checking the box to the left of the event: to delete checked rows from the history window, click the 'Remove Elements' 
                button. The 'Clear All ' button will delete all events from the history widow. 
                <p>
                Wildfires will occur at random in the future and will not be shown in the text box. If you select wildfire default, the historical frequency, size distribution, and severity will be used.  
                By specifying the Wildfire Suppression Level one can control the fire average interval between fires, but not the years wildfires occur. Total suppression will not allow 
                wildfires to occur. No suppression will allow the natural fire cycle typical in an area to occur. Typical fire suppression multiplies the average fire interval by 2.5x over that of the 
                natural fire cycle. If you select wildfire custom you may examine how changing fire characteristics influences carbon stores of forests.   The interval, severity, and size of fires may 
                all be varied from the most likely condition based on the region and elevation class selected.  At this point one may alter the fire interval continuously and shift severity from one 
                of the five classes to another.  Fire size can either be halved or doubled relative to the default value for a region and elevation class.   Bear in mind that fire size is described 
                by a distribution, so doubling fire size will increase the chances of a very large fire, whereas halving it will reduce the chances of a very large fire.  Since individual fire size 
                is random, one could have very large fires under either scenario. </p>
            ...
            <div id="inputForm">
                ...
                <tr><td>
                    ...
                    <asp:RangeValidator ID="valRange_priInterval" runat="server" 
                            ErrorMessage="RangeValidator" ControlToValidate="txt_priInterval" Text="*" 
                            SetFocusOnError="True" ValidationGroup="priValidators" Type="Integer"></asp:RangeValidator>
                    ...
                    &nbsp;<a href="docs/Carbon_Calculator_utilization.pdf"><asp:Image ID="img_help_priUtilization" runat="server" ImageUrl="~/images/help-about-icon.png" AlternateText="help" /></a>
                ...
                <td class="formLastDropdown">
                    ...
                    <asp:RequiredFieldValidator ID="valReq_priSeverity" 
                            ...
                            ValidationGroup="priValidators" InitialValue="Select Level" SetFocusOnError="True">*</asp:RequiredFieldValidator>
                    <a href="docs/Carbon_Calculator_Fire_History_Parameterization.pdf">
                    &nbsp;<asp:Image ID="img_help_pri_severity" runat="server" 
                        ImageUrl="~/images/help-about-icon.png" AlternateText="help" /></a>
                    <a href="#" onclick="helpWildfireLevel()">
                    <asp:Image ID="img_help_pri_suppression" runat="server" 
                ...
                <td>
                    ...
                    &nbsp;<a href="docs/Carbon_Calculator_Fire_History_Parameterization.pdf">
                    <asp:Image ID="img_help_secSeverity" runat="server" ImageUrl="~/images/help-about-icon.png" AlternateText="help" /></a>
                        <br />
                ...
                <table cellspacing="10px">
                 ...
                 <asp:ValidationSummary ID="ValidationSummary2" runat="server" 
                    ...
                    ShowSummary="False" />
                <br />
                <asp:Button ID="btn4previous" CssClass="formButton" runat="server" Text="Previous" 
                    OnClick="btn4previous_Click" TabIndex="12" />
                &nbsp;&nbsp;&nbsp;&nbsp;
                <asp:Button ID="btn4next" CssClass="formButton" runat="server" Text="Next" 
                    OnClick="btn4next_Click" TabIndex="13" />
                </form>
            </div>
        </div>
